...
void main(void)
{
    ...
    for (index = 0; index < 400; index++){
        senotable[index] = (uint16_t)(2500.0*(1.0 + sin(6.28318531/400.0*(float)index)));
        costable[index] =  (uint16_t)(2500.0*(1.0 + cos(6.28318531/400.0*(float)index)));
    }
...
void setup_epwm(void)
{
    ...
    EPwm7Regs.TBPRD                = 5000;              // PERIODO = Fosc/2*Fpwm OU Fosc/4*Fpwm
    ...
    EPwm8Regs.TBPRD                = 5000;              // PERIODO = Fosc/2*Fpwm OU Fosc/4*Fpwm
...
__interrupt void ISR_TIMER1(void)
{
    ...
    DacaRegs.DACVALS.all = offset1 + senotable[index] / 2;
    DacbRegs.DACVALS.all = offset2 +  costable[index] / 2;
...
__interrupt void ISR_ADC(void)
{
    ...
    EPwm8Regs.CMPA.bit.CMPA = senotable[index];
